-- Centang "sampai di perangkat" (✓✓ abu) yang AKURAT & bertahan.
--
-- Sebelumnya status delivered hanya disiarkan lewat WebSocket (efemeral): begitu
-- pengirim menutup aplikasi, informasinya hilang, sehingga di beranda centang
-- tak pernah bisa membedakan "terkirim ke server" (✓) dari "sampai di perangkat
-- lawan" (✓✓). Sekarang kita menyimpan pesan terakhir yang sudah diterima tiap
-- anggota, persis seperti last_read_message_id untuk centang biru.
--
-- Perbandingan memakai UUIDv7 yang terurut waktu, jadi id uuid bisa langsung
-- diadu dengan operator < (Postgres mengurutkan uuid sesuai byte, dan byte awal
-- v7 adalah timestamp).

BEGIN;

-- 1) Kolom baru: pesan terakhir yang sudah SAMPAI di perangkat anggota ini.
ALTER TABLE conversation_members
    ADD COLUMN IF NOT EXISTS last_delivered_message_id uuid;

-- 2) Tandai "sudah sampai" sampai p_message. Hanya maju (advance-only): tidak
--    pernah mundur, aman dipanggil berkali-kali dan di luar urutan. Tidak error
--    kalau bukan anggota — delivered bersifat best-effort.
...
BEGIN
    ...
    WHERE conversation_id = p_conversation
      AND user_id = v_user
      AND left_at IS NULL
      AND (last_delivered_message_id IS NULL OR last_delivered_message_id < p_message);
END;
$$;

REVOKE EXECUTE ON FUNCTION public.mark_conversation_delivered(uuid, uuid) FROM PUBLIC;
GRANT  EXECUTE ON FUNCTION public.mark_conversation_delivered(uuid, uuid) TO authenticated;

-- 3) list_conversations: tambahkan counterpart_last_delivered supaya beranda bisa
--    menggambar ✓ / ✓✓ / ✓✓ biru dengan akurat. Menambah kolom mengubah bentuk
--    baris → DROP dulu.
