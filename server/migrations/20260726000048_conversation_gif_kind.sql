-- Beranda chat: bedakan GIF dari foto biasa -> pratinjau [GIF].
--
-- list_conversations (migrasi 37) melaporkan KIND lampiran pesan terakhir supaya
-- beranda bisa menulis [Foto]/[Video]/[Pesan suara]. GIF tersimpan sebagai
-- media kind='image' (mime image/gif), jadi terbaca [Foto]. Di sini kind lampiran
-- dihitung: kalau mime image/gif ATAU storage_key berakhiran .gif -> 'gif',
-- selain itu tetap kind aslinya. App memetakan type 'gif' menjadi [GIF].
--
-- Mengubah isi kolom (bukan bentuk baris) -> CREATE OR REPLACE cukup; bentuk
-- RETURNS TABLE identik dengan migrasi 37.
